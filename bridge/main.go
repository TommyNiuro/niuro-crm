...
// msgCutoff is the earliest message timestamp the bridge will store. Messages
// older than this are ignored so the database only collects recent activity.
// Override with the WHATSAPP_SINCE env var (format YYYY-MM-DD). Default: 2026-01-01.
var msgCutoff = func() time.Time {
	s := os.Getenv("WHATSAPP_SINCE")
	if s == "" {
		s = "2026-01-01"
	}
...
// Estado del pairing expuesto en GET /api/qr para que el CRM muestre el QR en
// su propia UI (sin esto el QR solo sale por la terminal, inusable en la app).
var qrState = struct {
	sync.Mutex
	Status string // starting | qr | connected | timeout | logged_out
	Code   string
}{Status: "starting"}

func setQRState(status, code string) {
	qrState.Lock()
	qrState.Status = status
	qrState.Code = code
	qrState.Unlock()
}
...
// Extract text content from a message. Returns text content or a human-readable
// placeholder for non-text types so no message is silently dropped.
func extractTextContent(msg *waProto.Message) string {
	...
	if p := msg.GetPollCreationMessage(); p != nil {
		return "[encuesta: " + p.GetName() + "]"
	}
	if msg.GetPollUpdateMessage() != nil {
		return "[voto en encuesta]"
	}
	if loc := msg.GetLocationMessage(); loc != nil {
		return fmt.Sprintf("[ubicación: %.4f, %.4f]", loc.GetDegreesLatitude(), loc.GetDegreesLongitude())
	}
...
// resolveLID translates an @lid JID to its @s.whatsapp.net equivalent using the
// device store's lid_map table. Returns the original JID if no mapping is found.
func resolveLID(client *whatsmeow.Client, jid string) string {
	if !strings.HasSuffix(jid, "@lid") {
		return jid
	}
	lid := strings.TrimSuffix(jid, "@lid")
	// Group @lid JIDs contain a dash and are kept as-is.
	if strings.Contains(lid, "-") {
		return jid
	}
	pn, err := client.Store.LIDs.GetPNForLID(context.Background(), types.JID{
		User:   lid,
		Server: "lid",
	})
	if err != nil || pn.User == "" {
...
func handleMessage(client *whatsmeow.Client, messageStore *MessageStore, msg *events.Message, logger waLog.Logger) {
	...
	// Skip messages older than the configured cutoff (keeps the DB lean).
	if msg.Info.Timestamp.Before(msgCutoff) {
		return
	}
...
func startRESTServer(client *whatsmeow.Client, messageStore *MessageStore, port int) {
	...
	http.HandleFunc("/api/request-history-sync", requireAuth(authToken, func(w http.ResponseWriter, r *http.Request) {
		...
		if client.Store.ID == nil {
			w.WriteHeader(http.StatusServiceUnavailable)
			json.NewEncoder(w).Encode(map[string]interface{}{"success": false, "message": "Not logged in"})
			return
		}
		...
		if chatJID == "" {
			...
			json.NewEncoder(w).Encode(map[string]interface{}{"success": false, "message": "jid query param required"})
			return
		}

		// Find oldest message for this chat
		var msgID string
		var isFromMe bool
		var ts time.Time
		var tsStr string
		err := messageStore.db.QueryRow(
			...
			chatJID,
		).Scan(&msgID, &isFromMe, &tsStr)
		if err != nil {
			w.WriteHeader(http.StatusNotFound)
			json.NewEncoder(w).Encode(map[string]interface{}{"success": false, "message": "No messages for this chat: " + err.Error()})
			return
		}
		...
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			json.NewEncoder(w).Encode(map[string]interface{}{"success": false, "message": "Failed to parse timestamp: " + tsStr})
			return
		}
		...
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
			json.NewEncoder(w).Encode(map[string]interface{}{"success": false, "message": "Invalid JID: " + err.Error()})
			return
		}
		...
		historyMsg := client.BuildHistorySyncRequest(info, 100)
		if historyMsg == nil {
			w.WriteHeader(http.StatusInternalServerError)
			json.NewEncoder(w).Encode(map[string]interface{}{"success": false, "message": "Failed to build request"})
			return
		}

		myJID := types.JID{User: client.Store.ID.User, Server: "s.whatsapp.net"}
		_, err = client.SendMessage(context.Background(), myJID, historyMsg)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
	...
	// escucharia en todas las interfaces (0.0.0.0); "127.0.0.1:port" lo limita
	// al host. Los clientes (CRM, scripts, MCP) lo consumen por loopback.
...
func main() {
	...
	// Request a full history sync when this device links. Only takes effect on a
	// fresh QR pairing — it tells WhatsApp to push as much history as it allows
	// instead of just the recent window. Messages older than msgCutoff are still
	// dropped at store time, so this effectively backfills down to the cutoff.
	...
	// El REST server arranca ANTES del pairing: /api/qr tiene que responder
	// mientras se espera el escaneo (el resto de endpoints igual fallan limpio
	// hasta estar conectado). Puerto por env BRIDGE_PORT, default 8080.
	port := 8080
	...
	// (el REST server ya arranco antes del pairing)
...
func handleHistorySync(client *whatsmeow.Client, messageStore *MessageStore, historySync *events.HistorySync, logger waLog.Logger) {
	...
	for _, conversation := range historySync.Data.Conversations {
		...
		if len(messages) > 0 {
			...
			// Skip whole conversation if its latest message predates the cutoff.
			if timestamp.Before(msgCutoff) {
				continue
			}
			...
			for _, msg := range messages {
				...
				// Skip messages older than the configured cutoff.
				if timestamp.Before(msgCutoff) {
					continue
				}
