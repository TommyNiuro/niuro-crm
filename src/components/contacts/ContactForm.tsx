...
const contactSchema = z.object({
  ...
  notes: z.string(),
  monthlyValueUSD: z.number().min(0).optional(),
});
...
interface ContactFormProps {
  open: boolean;
  onClose: () => void;
  initialData?: Partial<ContactFormData> & { id?: string; valueCents?: number };
}
...
export function ContactForm({ open, onClose, initialData }: ContactFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      monthlyValueUSD: initialData?.valueCents ? initialData.valueCents / 100 : undefined,
    },
  });

  // Re-sincronizar el formulario al abrir o al cambiar de contacto: defaultValues
  // solo se evalúa al montar, así que editar otro contacto mostraba los datos del
  // anterior (auditoría 2026-06-09).
  useEffect(() => {
    if (!open) return;
    reset({
      name: initialData?.name || "",
  ...
  const onSubmit = async (data: ContactFormData) => {
    try {
      ...
      const payload = {
        ...data,
        valueCents: data.monthlyValueUSD != null ? Math.round(data.monthlyValueUSD * 100) : undefined,
        monthlyValueUSD: undefined,
      };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger id="temperature" className="cursor-pointer">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-2">
            <Label htmlFor="monthlyValueUSD">Valor mensual estimado (USD)</Label>
            <Input
              id="monthlyValueUSD"
              type="number"
              min="0"
              step="100"
              placeholder="3850"
              {...register("monthlyValueUSD", { valueAsNumber: true })}
            />
          </div>
