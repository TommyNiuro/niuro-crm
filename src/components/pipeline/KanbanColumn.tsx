...
export function KanbanColumn({ id, name, color, deals }: KanbanColumnProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div role="list" aria-label={`Deals en ${name}`} className="flex-1 p-2 space-y-2 min-h-[100px] overflow-y-auto">
          {deals.length === 0 ? (
            <p className="text-center text-[11px] text-muted-foreground py-6 px-2 leading-snug">
