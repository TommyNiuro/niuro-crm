...
import { STAGES, STAGE_CFG, STAGE_PERDIDOS } from "@/lib/crm-ui";
...
// Días que el contacto lleva en su etapa actual.
...
// Umbral de alerta: 3x el SLA de la etapa, mínimo 7 días.
function stageStaleThreshold(stage: string): number {
  const due = STAGE_CFG[stage]?.dueInDays ?? 2;
...
function ContactCard({
  ...
  const stale = stageDays != null && stageDays > stageStaleThreshold(c.stage);
  ...
  return (
    ...
    >
      {/* Row 1 — Avatar + name + channel + score */}
      ...
      {/* Row 2 — Temperature + last interaction */}
      ...
      {/* Row 3 — Next action or disqualify reason */}
      {lost ? (
        c.disqualifyReason ? (
      ...
      {/* Row 4 — Value + score bar + probability */}
...
export default function PipelinePage() {
  ...
  const move = async (id: string, stage: string) => {
    // Optimistic update CON revert (auditoría 2026-06-09: el .catch vacío dejaba
    // la UI mostrando un movimiento que el servidor nunca guardó).
  ...
  const staleCount     = activeContacts.filter((c) => {
    ...
    return d != null && d > stageStaleThreshold(c.stage);
  }).length;
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
        <div>
          <h1 className="text-[15px] font-semibold tracking-tight">Pipeline de Ventas</h1>
          <p className="text-[11px] text-muted-foreground mt-0.5">Arrastra los contactos entre etapas</p>
        </div>
      ...
      {loading ? (
        <div
          ...
          aria-busy="true"
          className="flex-1 p-5 flex gap-3"
        >
          {allCols.map((c) => (
        ...
        <div className="flex-1 min-h-0 flex gap-3 p-5 overflow-x-auto">
          {allCols.map(({ key: stage, isLost }) => {
            ...
            // Monto ponderado (HubSpot): suma de valor × probabilidad.
            ...
            // ── Columna colapsada: barra vertical estrecha ──
            if (isCollapsed) {
              return (
                <div
                  key={stage}
            ...
            return (
              ...
              >
                {/* Column header — estilo HubSpot: nombre + contador + colapsar */}
                <div
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2 min-h-[80px]">
                  {items.length === 0 ? (
                    <p className="text-center text-[11px] text-muted-foreground py-6 px-3 leading-snug">
                      {EMPTY_HINT[stage] ?? "Nada aquí todavía."}
                    </p>
                  ) : (
                    items.map((c) => (
                      <ContactCard
                ...
                {/* Footer estilo HubSpot — total + ponderado */}
