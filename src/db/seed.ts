...
// Guarda contra la DB de producción (auditoría 2026-06-09): el seed insertaba
// datos demo sobre datos reales sin preguntar.
try {
  const existing = sqlite.prepare("SELECT COUNT(*) AS c FROM contacts").get() as { c: number };
  if (existing.c > 0 && !process.argv.includes("--force")) {
    console.error(`La DB ya tiene ${existing.c} contactos reales — seed abortado. Usa --force solo si REALMENTE quieres insertar datos demo encima.`);
    process.exit(1);
  }
} catch { /* tabla aún no existe: DB fresca, seguir */ }
